...
POD=$(oc process metrics-deployer-template -n openshift -v HAWKULAR_METRICS_HOSTNAME=hawkular-metrics.example.com,USE_PERSISTENT_STORAGE=false,IMAGE_VERSION=latest  | oc create -f - | cut -d '"' -f2)
